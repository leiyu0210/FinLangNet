...
class Attention(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs, mask):
        ...
        for i in range(hidden.size(0)): 
            layer_hidden = hidden[i] 
            src_len = encoder_outputs.size(1)

            layer_hidden = layer_hidden.unsqueeze(1).repeat(1, src_len, 1)
            energy = torch.tanh(self.attn(torch.cat((layer_hidden, encoder_outputs), dim=2)))
            attention = self.v(energy).squeeze(2)  # [256, src_len]
            attention = attention.masked_fill(mask == 0, -1e10)

            attention_weights = F.softmax(attention, dim=1).unsqueeze(1)
            weighted = torch.bmm(attention_weights, encoder_outputs)  
            weighted = weighted.squeeze(1)  
            weighted_sum.append(weighted)

        weighted_sum = torch.stack(weighted_sum) 
...
class MyModel_GRU_Attention(nn.Module):
    def __init__(self,embedding_dim=16, hidden_dim=256, num_layers=2, bidirectional=False):
        ...
        self.attention_inquery = Attention(self.hidden_dim)
        self.attention_creditos = Attention(self.hidden_dim) 
        ...
        self.attention_time = Attention(self.hidden_dim)
        # Activation function
        self.act = F.gelu
    ...
    def forward(self, dz_categorica_feature, dz_numeric_feature, person_feature,len_dz,x_inquery, x_creditos,inquery_length,creditos_length):
        ...
        inquery_mask = torch.arange(x_inquery.size(0), device=device)[None, :] < inquery_length[:, None].to(device)
        hidden_inquery = self.attention_inquery(hidden_inquery, out_inquery, inquery_mask)
        ...
        creditos_mask = torch.arange(x_creditos.size(0), device=device)[None, :] < creditos_length[:, None].to(device)
        hidden_creditos = self.attention_creditos(hidden_creditos, out_creditos, creditos_mask)
        ...
        ts_mask = torch.arange(dz_feature_input.size(0), device=device)[None, :] < len_dz[:, None].to(device)
        time_hidden = self.attention_time(time_hidden, time_state, ts_mask)
