...
class MyModel_StackGRU(nn.Module):
    def __init__(self,embedding_dim=16, hidden_dim=256, num_layers=2, bidirectional=False):
        ...
        self.lstm_inquery_1layer = MaskedGRU(input_size= self.inquery_fe_len * self.embedding_dim , hidden_size = self.hidden_dim, num_layers= self.num_layers_circulo, batch_first=True, bidirectional= self.bidirectional)
        self.lstm_inquery_2layer = MaskedGRU(input_size= self.hidden_dim , hidden_size = self.hidden_dim, num_layers= self.num_layers_circulo, batch_first=True, bidirectional= self.bidirectional)

        
        self.lstm_creditos_1layer = MaskedGRU(input_size= self.creditos_fe_len * self.embedding_dim , hidden_size = self.hidden_dim, num_layers= self.num_layers_circulo, batch_first=True, bidirectional= self.bidirectional)
        self.lstm_creditos_2layer = MaskedGRU(input_size= self.hidden_dim , hidden_size = self.hidden_dim, num_layers= self.num_layers_circulo, batch_first=True, bidirectional= self.bidirectional)
        ...
        # time model
        self.lstm_time_1layer = MaskedGRU(input_size= self.category_fe_len * self.embedding_dim , hidden_size = self.hidden_dim, num_layers= self.num_layers_circulo, batch_first=True, bidirectional= self.bidirectional)
        self.lstm_time_2layer = MaskedGRU(input_size= self.hidden_dim , hidden_size = self.hidden_dim, num_layers= self.num_layers_circulo, batch_first=True, bidirectional= self.bidirectional)
    ...
    def forward(self, dz_categorica_feature, dz_numeric_feature, person_feature,len_dz,x_inquery, x_creditos,inquery_length,creditos_length):
        ...
        out_inquery_1layer, hidden_inquery_1layer = self.lstm_inquery_1layer(inquery_concatenated_features, inquery_length)
        out_inquery, hidden_inquery = self.lstm_inquery_2layer(out_inquery_1layer, inquery_length)
        ...
        out_creditos_1layer, hidden_creditos_1layer = self.lstm_creditos_1layer(creditos_concatenated_features, creditos_length)
        out_creditos, hidden_creditos = self.lstm_creditos_2layer(out_creditos_1layer, creditos_length)
        ...
        out_time_1layer, hidden_time_1layer = self.lstm_time_1layer(time_input_features, len_dz)
        out_time, time_hidden = self.lstm_time_2layer(out_time_1layer, len_dz)
