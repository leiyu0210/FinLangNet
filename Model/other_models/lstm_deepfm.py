...
class MyModel_LSTM(nn.Module):
    def __init__(self,embedding_dim=16, hidden_dim=256, num_layers=2, bidirectional=False):
        ...
        self.lstm_inquery = MaskedLSTM(input_size= self.inquery_fe_len * self.embedding_dim , hidden_size = self.hidden_dim, num_layers= self.num_layers_circulo, batch_first=True, bidirectional= self.bidirectional)

        
        self.lstm_creditos = MaskedLSTM(input_size= self.creditos_fe_len * self.embedding_dim   , hidden_size= self.hidden_dim, num_layers=self.num_layers_circulo, batch_first=True, bidirectional= self.bidirectional)
        ...
        # Embedding and encoder for time feature
         # time model
        self.lstm = MaskedLSTM(input_size= self.category_fe_len * self.embedding_dim , hidden_size= self.hidden_dim, num_layers= self.num_layers, batch_first=True, bidirectional= self.bidirectional)  
